...
namespace MangaManagementSystem.Infrastructure.Repositories
{
    public class ChapterPageTaskRepository : GenericRepository<ChapterPageTask>, IChapterPageTaskRepository
    {
        ...
        {
            var normalizedRegionIds =
                (pageRegionIds ?? Array.Empty<Guid>())
                    .Distinct()
                    .ToArray();
            ...
            {
                var regions =
                    await _context.PageRegions
                        .Where(region =>
                            normalizedRegionIds.Contains(
                                region.PageRegionId))
                        .Include(region =>
                            region.ChapterPageVersion)
                            .ThenInclude(version =>
                                version!.ChapterPage)
                                .ThenInclude(page =>
                                    page!.Chapter)
                ...
                var pageVersion =
                    regions[0].ChapterPageVersion;

                var page =
                    pageVersion?.ChapterPage;
                ...
                var contributorUserIds =
                    await _context.SeriesContributors
                        .Where(contributor =>
                ...
                if (!SeriesProductionPolicy
                        .AllowsNormalProduction(series.StatusCode)
                    || !ChapterPageTaskCreationPolicy
                ...
                when (IsExpectedTaskConstraintFailure(ex))
            {
                if (ownedTransaction != null)
                {
                ...
                _logger.LogWarning(
                    ex,
                    "Expected database constraint failure while creating a chapter page task.");
            ...
            catch (DbUpdateException ex)
            {
                ...
                _logger.LogError(
                    ex,
                    "Unexpected database failure while creating a chapter page task.");
