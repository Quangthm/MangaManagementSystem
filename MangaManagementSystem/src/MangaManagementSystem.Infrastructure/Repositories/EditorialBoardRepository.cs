...
public sealed class EditorialBoardRepository : IEditorialBoardRepository
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                throw new InvalidOperationException("Proposal not found or cannot be opened for board poll.");
            }
            ...
            AddNullableStringParameter(command, "@publicationFrequencyCode", request.PublicationFrequencyCode, 50);
        ...
        catch (SqlException ex) when (ex.Number is 2601 or 2627)
        {
            ...
            throw new InvalidOperationException(
                "This series already has an open poll with the same poll type.",
                ex);
        }
        catch
        {
    ...
    private static void ValidateOpenPollRequest(OpenSeriesBoardPollRequestDto request)
    {
        ...
        {
            throw new InvalidOperationException("Publication frequency is required for START_SERIALIZATION poll.");
        }

        if (request.PollTypeCode == "CANCEL_SERIALIZATION" &&
            !string.IsNullOrWhiteSpace(request.PublicationFrequencyCode))
        {
            throw new InvalidOperationException("Publication frequency must be empty for CANCEL_SERIALIZATION poll.");
        }
    ...
    private static void ValidateVoteRequest(CastSeriesBoardVoteRequestDto request)
    {
        ...
        if (request.ChoiceCode == "REJECT" &&
            string.IsNullOrWhiteSpace(request.VoteReason))
        {
