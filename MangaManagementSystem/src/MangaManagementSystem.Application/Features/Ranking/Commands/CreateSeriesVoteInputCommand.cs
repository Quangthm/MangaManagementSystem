...
public sealed record CreateSeriesVoteInputCommand(
    Guid ActorUserId,
...
public sealed class CreateSeriesVoteInputCommandHandler
    : IRequestHandler<CreateSeriesVoteInputCommand, SeriesVoteInputDto>
{
    ...
    public async Task<SeriesVoteInputDto> Handle(
        CreateSeriesVoteInputCommand request,
    ...
    private async Task ValidateActorAsync(Guid actorUserId, CancellationToken cancellationToken)
    {
        if (!await _repository.IsVoteInputActorAsync(actorUserId, cancellationToken))
        {
            throw new UnauthorizedAccessException(
                "Only Editorial Board Members or Editorial Board Chief users may enter series vote input.");
        }
    }
