...
public sealed class GetMySeriesProposalDetailQueryHandler
    : IRequestHandler<GetMySeriesProposalDetailQuery, MangakaSeriesProposalDto?>
{
    private readonly ISeriesProposalRepository _seriesProposalRepository;

    public GetMySeriesProposalDetailQueryHandler(ISeriesProposalRepository seriesProposalRepository)
    {
        _seriesProposalRepository = seriesProposalRepository;
    }

    public async Task<MangakaSeriesProposalDto?> Handle(
        GetMySeriesProposalDetailQuery request,
        CancellationToken cancellationToken)
    {
        if (request.ActorUserId == Guid.Empty)
            return null;

        var proposal = await _seriesProposalRepository.GetMySeriesProposalDetailAsync(
            request.ActorUserId,
            request.SeriesProposalId,
            cancellationToken);
