export function createMangaCanvasInstance() {
  ...
  // Where the current region-drag started (image space) + how far (screen px) the pointer must travel
  // before it counts as a real move. Below the threshold a press is a pure click: the box is not nudged
  // and mouseup skips syncToBlazor(), so a (possibly jittery) double-click to SELECT never emits a
  // competing "0 selected" update that would race the dblclick's "1 selected" and unselect the region.
  ...
  function setupEvents() {
    ...
    container.addEventListener("mousedown", (e) => {
      if (currentTool === "pan" || e.button === 1 || isSpacePressed) {
        // Middle click or Space to pan
        isPanning = true;
      ...
      if (currentTool === "draw") {
        ...
        if (clickedHandle && regionsEditable) {
          ...
          originalRegionRect = {
            x: clickedRegion.x,
            y: clickedRegion.y,
            w: clickedRegion.width,
            h: clickedRegion.height,
          };
          return;
        }
    ...
    container.addEventListener("mousemove", (e) => {
      ...
      if (currentTool === "select") {
        if (isDraggingRegion && targetRegion) {
          // Dead zone: until the pointer travels past DRAG_THRESHOLD_PX this press is still just a click
          // (or one half of a double-click). Bail out before touching the box or setting hasDragged, so
          // a stationary/jittery select never moves the region, marks it dirty, or triggers a mouseup
          // sync. Distance is measured in screen px (image-space delta * scale).
          if (!hasDragged && dragStartPos) {
            const movedPx =
              Math.hypot(pos.x - dragStartPos.x, pos.y - dragStartPos.y) * scale;
          ...
          if (imgW != null)
            nx = Math.max(0, Math.min(nx, imgW - targetRegion.width));
          if (imgH != null)
            ny = Math.max(0, Math.min(ny, imgH - targetRegion.height));
  ...
  function fitImageOntoCanvas(image, attempt) {
    ...
    const safeW = cw || image.width,
      safeH = ch || image.height;
  ...
  function loadImage(dataUrl) {
    ...
    return new Promise((resolve) => {
      ...
      img.onerror = () => {
        ...
        img2.onerror = () => {
          console.error(
            "Failed to load image (with and without CORS):",
            dataUrl,
          );
          resolve();
        };
  ...
  function resetZoom() {
    if (!originalImg) return;
    scale = Math.min(
      container.clientWidth / originalImg.width,
      container.clientHeight / originalImg.height,
    );
  ...
  function loadRegions(savedRegionsStr, silent) {
    ...
    nextId =
      regions.reduce(
        (mx, r) => Math.max(mx, r && typeof r.id === "number" ? r.id : 0),
        0,
      ) + 1;
    ...
    saveState(true);
    if (!silent && typeof dotNetRef !== "undefined" && dotNetRef)
      syncToBlazor();
    redraw();
  }
  ...
  function selectRegionsByDbIds(dbIds) {
    ...
    regions.forEach((r) => {
      r.selected = r.dbId != null && set.has(String(r.dbId));
    });
    syncToBlazor();
    redraw();
  }
  ...
  function undo() {
    if (historyIndex > 0) {
      ...
      try {
        currentImgSrc = backgroundCanvas.toDataURL("image/png");
      } catch (e) {
        /* tainted */
      }
  ...
  function redo() {
    if (historyIndex < historyStack.length - 1) {
      ...
      try {
        currentImgSrc2 = backgroundCanvas.toDataURL("image/png");
      } catch (e) {
        /* tainted */
      }
  ...
  async function callSegmentAPI() {
    ...
    try {
      ...
      if (!dotNetRef)
        return "The page is not ready. Please reload and try again.";
      ...
      const resultStr = await dotNetRef.invokeMethodAsync(
        "SegmentImageJS",
        base64Data,
      );
...
window.setUnsavedFlag = function (flag) {
  window.mmsHasUnsaved = !!flag;
};
