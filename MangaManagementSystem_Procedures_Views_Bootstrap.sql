...
GO
CREATE OR ALTER VIEW dbo.PourSession_ML_Features
AS
WITH FlowLogs AS (
    SELECT
        sl.session_id,
    ...
    FROM dbo.SensorLog sl
    INNER JOIN dbo.SensorType st
        ON st.sensor_type_id = sl.sensor_type_id
    LEFT JOIN dbo.SensorLogMeta slm
        ON slm.log_id = sl.log_id
    WHERE st.sensor_name = N'FLOW'
),
...
FlowAgg AS (
    SELECT
        session_id,
        COUNT(flow_value_ml_s) AS flow_sample_count,
        AVG(flow_value_ml_s) AS avg_flow_sample,
        MAX(flow_value_ml_s) AS peak_flow_sample,
        MIN(flow_value_ml_s) AS min_flow_sample,
        STDEV(flow_value_ml_s) AS std_flow_sample
    FROM FlowLogs
    WHERE flow_value_ml_s IS NOT NULL
    GROUP BY session_id
)
...
SELECT
    ...
    -- Derived
    CASE 
        WHEN ps.duration_s > 0 
        THEN ps.actual_ml / ps.duration_s 
        ELSE 0 
    END AS avg_flow,
    ...
    fa.min_flow_sample,
    fa.std_flow_sample,
    fa.flow_sample_count,

    -- Behavior context
    ps.start_reason,
    ps.stop_reason,

    -- Label candidate (for supervised learning)
    ps.result_code

FROM dbo.PourSession ps
LEFT JOIN FlowAgg fa
    ON ps.session_id = fa.session_id;
Go
