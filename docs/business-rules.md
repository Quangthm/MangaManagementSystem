...
> **Latest alignment update — 2026-07-02:** This version incorporates the finalized chapter-level publication scheduling direction: planned release dates belong to `Chapter`, approved chapters with valid planned dates become `SCHEDULED`, scheduled/on-hold chapters lock Mangaka and page mutation workflows, editor rescheduling must follow publication-period rules, and first planned chapters use the current/next weekly or current monthly period rule. It also preserves the series/slug/workspace direction: normal series draft creation and editing belongs to Mangaka, not Admin; draft series are managed internally by ID; slug is generated by the backend from title during `PROPOSAL_DRAFT` and locks after the series leaves draft; `/series/{slug}` is the stable main series URL after serialization; the centralized authorized workspace is chapter-level with left navigation for chapters/pages/versions and a right tools/actions panel; AI tools are available to all Authorized Page Workspace Users, while role-specific business actions remain permission-gated. It also incorporates the page annotation junction-table update: one `ChapterPageAnnotation` can link to one or more `PageRegion` records through `ChapterPageAnnotationRegion`, and resolving annotations preserves the original feedback and links.
...
| BR-CH-014 | Mangaka and Tantou Editors may set `Chapter.planned_release_date` while the chapter is still in a plannable or editable state such as `DRAFT`, `REVISION_REQUESTED`, or `APPROVED`, subject to publication-frequency validation. | Active draft |
| BR-CH-015 | If an `APPROVED` chapter receives a valid `planned_release_date`, the chapter should move to `SCHEDULED`. | Active draft |
...
| BR-CH-017 | When a chapter is `SCHEDULED`, a Tantou Editor may reschedule `planned_release_date` if the new date still satisfies the allowed `PublicationPeriod` rule for the series' publication frequency. | Active draft |
| BR-CH-018 | When a chapter is `SCHEDULED`, a Tantou Editor may move the chapter to `ON_HOLD` with a required reason; recovery from `ON_HOLD` is intentionally deferred outside this finalized scheduling task. | Active draft |
...
| BR-PUB-005 | `publication_frequency_code` may be `NULL` before the release approach has been decided; when set during `PROPOSAL_DRAFT`, it represents the Mangaka's proposed/preferred publication frequency. | Active draft |
...
| Rule ID | Business Rule | Review Status |
|---|---|---|
| BR-PUB-SCHEDULE-001 | Scheduling validation uses the latest non-cancelled chapter in the same series that has a `planned_release_date`, excluding the current chapter. | Active draft |
| BR-PUB-SCHEDULE-002 | If a previous planned non-cancelled chapter exists for a `WEEKLY` series, the new chapter `planned_release_date` must fall inside the next weekly `PublicationPeriod` after the previous chapter's weekly `PublicationPeriod`. | Active draft |
| BR-PUB-SCHEDULE-003 | If a previous planned non-cancelled chapter exists for a `MONTHLY` series, the new chapter `planned_release_date` must fall inside the next monthly `PublicationPeriod` after the previous chapter's monthly `PublicationPeriod`. | Active draft |
| BR-PUB-SCHEDULE-004 | If no previous planned non-cancelled chapter exists, the current chapter is treated as the first planned chapter for the series. | Active draft |
| BR-PUB-SCHEDULE-005 | For the first planned chapter of a `WEEKLY` series, the planned release date may fall inside either the current weekly `PublicationPeriod` or the next weekly `PublicationPeriod`. | Active draft |
| BR-PUB-SCHEDULE-006 | For the first planned chapter of a `MONTHLY` series, the planned release date must fall inside the current monthly `PublicationPeriod`. | Active draft |
| BR-PUB-SCHEDULE-007 | For an `IRREGULAR` series, the system does not enforce next-week or next-month planned release date rules. | Active draft |
| BR-PUB-SCHEDULE-008 | If `Series.publication_frequency_code` is `NULL`, the system may allow scheduling without strict weekly/monthly period validation unless a later workflow explicitly requires stricter behavior. | Active draft |
| BR-PUB-SCHEDULE-009 | Weekly default scheduling may use previous planned release date + 7 days when a previous planned chapter exists; for the first planned weekly chapter, the UI may suggest the current or next weekly period. | Active draft |
| BR-PUB-SCHEDULE-010 | Monthly default scheduling may use the same day number in the next month or the last day of that month when a previous planned chapter exists; for the first planned monthly chapter, the UI should guide the user to the current monthly period. | Active draft |
| BR-PUB-SCHEDULE-011 | Publication frequency enforcement is based on planned release dates, while actual release reporting uses `released_at_utc` converted to the system publication timezone. | Active draft |
| BR-PUB-SCHEDULE-012 | If a chapter is released late, the actual release date may be reported in a later `PublicationPeriod`, but this does not automatically shift the planned schedule of the next chapter unless an authorized user reschedules it. | Active draft |

### 17.5 Scheduled Chapter Lock, Reschedule, and Hold
...
| BR-PUB-SCHEDULED-003 | Tantou Editors may reschedule a `SCHEDULED` chapter by changing `Chapter.planned_release_date` if the new date satisfies the publication-frequency validation rules. | Active draft |
...
| BR-PUB-SCHEDULED-005 | Moving a chapter to `ON_HOLD` does not define the recovery workflow; resuming, unscheduling, or releasing from `ON_HOLD` is deferred to a later task. | Active draft |
| BR-PUB-SCHEDULED-006 | Schedule setting, rescheduling, automatic approval-to-scheduled transitions, and hold actions should be audit-visible, including actor, old/new status, old/new planned release date, and reason when applicable. | Active draft |
