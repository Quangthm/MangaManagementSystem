...
| US-PAGE-010 | BR-ANN-015, BR-ANN-016, BR-ANN-017 | As an Authorized Page Workspace User, I want to resolve annotations without deleting them or their region links, so that completed feedback remains traceable. |

## 5. Mangaka

| Story ID | Source Rule(s) | User Story |
|---|---|---|
| US-MANGAKA-001 | BR-SERIES-003, BR-SERIES-005, BR-SERIES-006, BR-SERIES-007, BR-SERIES-008, BR-SERIES-009, BR-SERIES-015, BR-SERIES-022, BR-SERIES-025 | As a Mangaka, I want to create and update a series draft profile with title, synopsis, genre, language, cover image, source series, and proposed publication frequency while it is still in `PROPOSAL_DRAFT`, so that the manga project is clearly represented before formal review. |
...
| US-EDITOR-006 | BR-ANN-001, BR-ANN-002, BR-ANN-003, BR-ANN-004, BR-ANN-005, BR-ANN-006 | As a Tantou Editor, I want to create annotations linked to one or more page regions, so that page-specific, multi-region, and whole-page feedback can be recorded without storing direct annotation coordinates. |
...
| US-EDITOR-008 | BR-ANN-015, BR-ANN-016, BR-ANN-017 | As a Tantou Editor, I want to mark annotations as resolved without deleting them, so that completed feedback can be distinguished from unresolved feedback while remaining traceable. |
