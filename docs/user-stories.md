...
> **Latest alignment update — 2026-07-04:** This version updates publication scheduling stories to the advisory scheduling model. Publication frequency now provides suggestions and warnings, not hard date enforcement; Mangaka and Tantou Editors may schedule/reschedule planned release dates on the current publication business date or later when allowed; Editors enforce hold/release actions; on-hold chapters require a new planned date to return to schedule; and auto-hold/release automation remain deferred.
...
> **Latest ranking and notification alignment — 2026-07-21:** Existing exact-series notification scoping, task reassignment notices, board decisions, publication-schedule exclusions, and account-approval notification/email behavior remain unchanged. Ranking uses the weighted formula `(v / (v + m)) * R + (m / (v + m)) * C`, with `reading_count` kept as separate popularity evidence. `RANKING_WARNING` uses the approved hybrid weekly rule: a completed week fails only when both `ranking_score < 6.5` and bottom-25% rank are true; high risk requires failure in at least 2 of the latest 3 consecutive completed weeks including the latest. All distinct active contributors of the exact affected series receive the advisory warning.

> **Latest chapter submission validation — 2026-07-21:** Mangaka chapter submission is allowed only from `DRAFT` or `REVISION_REQUESTED` when zero distinct associated page tasks remain `ASSIGNED` or `UNDER_REVIEW`. `COMPLETED` and `CANCELLED` tasks do not block. When active work remains, the backend blocks cleanly without changing chapter/task state or producing successful submission audit/notification behavior.
...
| Authorized Page Workspace User | A user who is permitted to access the chapter-level workspace for viewing and, where separately authorized by role/state, page-level editing, review, annotation, AI segmentation, AI/OCR translation support, or page-version feedback. Read access does not imply write permission. This normally includes Mangaka and Tantou Editor, and may include Assistant only for assigned page/task work. Editorial Board Members are excluded unless explicitly granted page workspace permissions. |
...
| US-NEW-005 | BR-USER-023, BR-USER-024 | As a New User registering with email/password, I want to pass reCAPTCHA and verify the 6-digit code sent to my email before my pending account is created, so that public registration verifies a human user and the entered email without bypassing Admin approval. |
...
| US-USER-013 | BR-NOTIF-007, BR-NOTIF-008, BR-NOTIF-027, BR-NOTIF-028, BR-NOTIF-029, BR-NOTIF-030, BR-NOTIF-032 | As an active contributor of a `SERIALIZED` or `HIATUS` series, I want to receive an advisory `RANKING_WARNING` only when the series persistently fails both the weighted-score baseline (`ranking_score < 6.5`) and bottom-25% weekly rank checks, so that the team is warned about sustained low performance without false warnings caused by relative rank alone or automatic cancellation. |
...
| US-MANGAKA-008 | BR-CH-001, BR-CH-002, BR-CH-003, BR-CH-006, BR-CH-014, BR-PUB-SCHEDULE-001, BR-PUB-SCHEDULE-005, BR-PUB-SCHEDULE-006 | As a Mangaka, I want to create or plan chapters with planned release dates of today or later and helpful frequency suggestions, so that active series chapters remain organized while still allowing realistic release exceptions. |
...
| US-MANGAKA-018 | BR-CH-SUB-001, BR-CH-SUB-002, BR-CH-SUB-005, BR-CH-SUB-007, BR-CH-SUB-008, BR-CH-SUB-009, BR-CH-SUB-013 | As a Mangaka contributor, I want to submit a `DRAFT` or `REVISION_REQUESTED` chapter for editorial review only after required active page versions exist and zero distinct associated page tasks remain `ASSIGNED` or `UNDER_REVIEW`, so that the Tantou Editor receives a stable chapter after Assistant production work is resolved. |
| US-MANGAKA-018A | BR-CH-SUB-008, BR-CH-SUB-010, BR-CH-SUB-011, BR-CH-SUB-012 | As a Mangaka contributor, I want the system to block chapter submission with a clear message when page tasks are still `ASSIGNED` or `UNDER_REVIEW`, without automatically changing those tasks or pretending submission succeeded, so that I know I must complete or cancel active work before retrying. |
...
| US-MANGAKA-022 | BR-RANK-004, BR-RANK-005, BR-RANK-008, BR-RANK-009, BR-NOTIF-007, BR-NOTIF-008 | As a Mangaka, I want to track weighted ranking trends and cancellation-risk evidence, including completed series when ranking input exists, so that I can respond to performance issues while historical performance remains visible and ranking warnings require both the approved low-score and bottom-25% checks with persistent weekly evidence. |
...
| US-MANGAKA-026 | BR-PUB-SCHEDULE-001, BR-PUB-SCHEDULE-002, BR-PUB-SCHEDULE-003, BR-PUB-SCHEDULE-004, BR-PUB-SCHEDULE-005, BR-PUB-SCHEDULE-006, BR-PUB-SCHEDULE-007, BR-PUB-SCHEDULE-010 | As a Mangaka, I want frequency-based suggested dates and warnings while still being allowed to choose any planned release date on the current publication business date or later, so that I can coordinate realistic release timing with my editor. |
...
| US-ASSISTANT-008 | BR-RANK-001, BR-RANK-003, BR-RANK-005, BR-RANK-009, BR-RANK-013 | As an Assistant, I want to view dynamic series rankings, including tied rank positions and completed-series history when ranking input exists, so that I can see publication-performance context without gaining permission to edit ranking input. |
...
| US-EDITOR-001B | BR-WORKSPACE-013, BR-WORKSPACE-014, BR-REG-036 | As an active Tantou Editor contributor, I want to view chapter, page, page-version, region, and annotation context for my exact series even when the chapter is `DRAFT` or another read-only state, so that I can understand upcoming or historical work without receiving mutation authority before/after the permitted review states. |
...
| US-EDITOR-004 | BR-REG-019, BR-REG-020, BR-REG-022, BR-REG-023, BR-REG-026, BR-REG-027, BR-REG-035, BR-REG-036, BR-CP-018, BR-CP-028 | As an active Tantou Editor contributor, I want to create or edit PageRegions for editorial review only while a chapter is `UNDER_REVIEW` or `REVISION_REQUESTED`, including changing permitted geometry and `type_code` or saving accepted AI suggestions, so that my review feedback can target accurate page areas without changing the underlying manga page content. |
...
| US-EDITOR-006 | BR-ANN-001, BR-ANN-002, BR-ANN-003, BR-ANN-004, BR-ANN-005, BR-ANN-006, BR-ANN-013, BR-ANN-014, BR-ANN-027, BR-REG-035 | As a Tantou Editor, I want to create editorial-review annotations in permitted review states using existing saved PageRegions, newly created PageRegions, or both, so that page-specific, multi-region, and whole-page feedback can be recorded without storing direct annotation coordinates. |
...
| US-EDITOR-008A | BR-ANN-023, BR-ANN-024, BR-ANN-026, BR-ANN-027 | As a Tantou Editor, I want to update unresolved annotation text when clarification is needed during `UNDER_REVIEW` or `REVISION_REQUESTED`, while retaining read-only visibility in other chapter states, so that feedback can be clarified without expanding Editor write authority outside the review workflow. |
...
| US-EDITOR-010 | BR-CH-REV-006, BR-CH-REV-007, BR-CH-REV-008, BR-CH-REV-010, BR-CH-REV-011, BR-CH-REV-011A, BR-CH-REV-012, BR-CH-REV-013, BR-CH-REV-015 | As a Tantou Editor, I want to record APPROVED, REVISION_REQUESTED, or CANCELLED decisions, with approval moving a chapter to SCHEDULED when a planned release date on the current publication business date or later already exists, so that chapter outcomes are clear and enforceable. |
...
| US-EDITOR-012 | BR-CH-010, BR-CH-018, BR-PUB-SCHEDULED-004, BR-PUB-SCHEDULED-005, BR-PUB-SCHEDULED-006 | As a Tantou Editor, I want to place a scheduled chapter ON_HOLD with a reason and later return it to schedule only with a new planned date on the current publication business date or later, so that paused work can resume with an explicit new plan. |
| US-EDITOR-014 | BR-CH-017, BR-PUB-SCHEDULED-003, BR-PUB-SCHEDULE-001, BR-PUB-SCHEDULE-005, BR-PUB-SCHEDULE-006, BR-PUB-SCHEDULE-007, BR-PUB-SCHEDULE-010 | As a Tantou Editor, I want to reschedule chapters to any planned date on the current publication business date or later with advisory frequency warnings and confirmation, so that release planning can handle bulk releases, breaks, vacations, and editorial exceptions. |
...
| US-BOARD-009 | BR-SERIES-VOTE-001, BR-SERIES-VOTE-002, BR-SERIES-VOTE-003, BR-SERIES-VOTE-004, BR-SERIES-VOTE-005, BR-SERIES-VOTE-006, BR-SERIES-VOTE-007, BR-SERIES-VOTE-008, BR-SERIES-VOTE-009, BR-SERIES-VOTE-010, BR-SERIES-VOTE-013, BR-SERIES-VOTE-014, BR-SERIES-VOTE-015, BR-SERIES-VOTE-016 | As an Editorial Board Member, I want to enter one simulated or aggregated series vote input for a series and weekly publication period with valid rating count, average rating, reading count, source note, timestamp, and entered-by user, so that ranking can be demonstrated without a public reader module while broader ranking views are derived from weekly evidence. |
