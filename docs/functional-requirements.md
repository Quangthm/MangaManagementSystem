...
| FR-CH-014 | The system shall allow Mangaka and Tantou Editors to set `Chapter.planned_release_date` while the chapter is in a plannable or editable state, subject to publication-frequency validation. | BR-CH-014 |
| FR-CH-015 | The system shall move an `APPROVED` chapter to `SCHEDULED` when a valid `planned_release_date` is set. | BR-CH-015 |
| FR-CH-016 | The system shall lock Mangaka chapter content changes when a chapter is `SCHEDULED`. | BR-CH-016 |
| FR-CH-017 | The system shall allow Tantou Editors to reschedule a `SCHEDULED` chapter only when the new planned release date satisfies the publication-frequency period rule. | BR-CH-017 |
| FR-CH-018 | The system shall defer recovery from `ON_HOLD` to a later workflow. | BR-CH-018 |
...
| FR-PUB-SCHEDULE-001 | The system shall find the previous planned chapter by selecting the latest non-cancelled chapter in the same series with `planned_release_date`, excluding the current chapter. | BR-PUB-SCHEDULE-001 |
| FR-PUB-SCHEDULE-002 | The system shall require a non-first `WEEKLY` series chapter to fall inside the next weekly `PublicationPeriod` after the previous planned chapter's weekly period. | BR-PUB-SCHEDULE-002 |
| FR-PUB-SCHEDULE-003 | The system shall require a non-first `MONTHLY` series chapter to fall inside the next monthly `PublicationPeriod` after the previous planned chapter's monthly period. | BR-PUB-SCHEDULE-003 |
| FR-PUB-SCHEDULE-004 | The system shall treat a chapter as the first planned chapter when no previous planned non-cancelled chapter exists in the same series. | BR-PUB-SCHEDULE-004 |
| FR-PUB-SCHEDULE-005 | The system shall allow the first planned chapter of a `WEEKLY` series to be scheduled inside either the current weekly `PublicationPeriod` or the next weekly `PublicationPeriod`. | BR-PUB-SCHEDULE-005 |
| FR-PUB-SCHEDULE-006 | The system shall require the first planned chapter of a `MONTHLY` series to be scheduled inside the current monthly `PublicationPeriod`. | BR-PUB-SCHEDULE-006 |
| FR-PUB-SCHEDULE-007 | The system shall not enforce weekly or monthly period boundaries for `IRREGULAR` series. | BR-PUB-SCHEDULE-007 |
| FR-PUB-SCHEDULE-008 | The system may allow scheduling without strict period validation when official frequency is `NULL`, unless a later workflow defines stricter behavior. | BR-PUB-SCHEDULE-008 |
| FR-PUB-SCHEDULE-009 | The system shall not automatically shift future planned schedule periods when a chapter is released late unless an authorized user reschedules it. | BR-PUB-SCHEDULE-012 |
...
| FR-PUB-SCHEDULED-003 | The system shall allow Tantou Editors to reschedule a `SCHEDULED` chapter only when the new date satisfies the publication-frequency validation rules. | BR-PUB-SCHEDULED-003 |
...
| FR-PUB-SCHEDULED-005 | The system shall defer `ON_HOLD` recovery, unscheduling, release automation, and public release visibility to later workflows. | BR-PUB-SCHEDULED-005 |
| FR-PUB-SCHEDULED-006 | The system shall audit schedule setting, rescheduling, approval-to-scheduled transitions, and hold actions with relevant old/new status, old/new date, actor, and reason details. | BR-PUB-SCHEDULED-006 |
