...
> - **#4b — CHƯA làm (editor duyệt chapter):** API sẵn `IEditorChapterReviewApiClient.SubmitReviewDecisionAsync(actorUserId, chapterId, SubmitChapterEditorialReviewRequest{decisionCode: APPROVED/REVISION_REQUESTED/CANCELLED, comments})`, chỉ cho chapter `UNDER_REVIEW`. Plan: inject client vào workspace → thêm panel/nút "Submit Review Decision" cho editor khi chapter UNDER_REVIEW (Approve / Request Revision / Cancel + comment) → gọi API. Cân nhắc dùng `SubmitReviewDecisionWithMarkupAsync` nếu cần đính markup. Đây là feature riêng, nên làm ở turn tập trung + test runtime.
>
