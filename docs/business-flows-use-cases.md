...
## Phase 7 Addendum — Auth/Admin Account Management Authorization Boundary
...
## BF-AUTH-007 — Public Admin Registration Request Remains Pending
...
```text
UI register form
→ User selects role_name = Admin
→ Backend API receives registration request
→ C# Application validates Admin is in the public registration whitelist
→ C# Application starts the normal registration/OTP workflow
→ C# Application hashes password after OTP completion
→ Infrastructure calls auth.usp_User_Create or auth.usp_User_CreateWithOptionalPortfolio
→ Database inserts auth.Users row with role = Admin and status_code = PENDING_APPROVAL
→ Database writes USER_REGISTERED audit event
→ Backend returns pending approval response
→ UI shows pending approval page/message
```
...
## BF-AUTH-008 — Direct Internal Google Signup Request Is Rejected
...
→ Request does not contain valid X-Internal-Api-Key
→ API rejects the request before Application command processing
→ No user account is created
→ No role is assigned
→ No pending or active account is inserted
...
## BF-AUTH-009 — Direct URL Authorization Cannot Bypass Role or Status
...
→ User types an Admin URL directly, such as /admin or /admin/users
→ Web route authorization checks authentication, role, and account status
→ API endpoints also validate Admin actor authorization where required
→ If user is not an active Admin, access is denied
→ UI redirects to login, pending approval, or access denied as appropriate
```
